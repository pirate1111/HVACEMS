					...
					<div class="row">
						<div class="input-group" style="width: 800px;">
							...
							<select class="form-control" name="timeRange" id="timeRange" onchange="if(this.value) location.href=(this.value);">
								<option>연별</option>
								<option value="search.do">월별</option>
							</select>&nbsp;&nbsp; 
							...
							<input type="text" class="form-control datetimepicker"
								...
								<button type="button" class="btn bg-gradient-success"
									onclick="searchList_go(1);" data-card-widget="searchyear"
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-12">
						<div class="callout callout-info">
							<h5 style="text-align: center;"> ${command.sectionType} 연도별 전력소비량 조회</h5>
						</div>
					</div>
				</div>
				<br>
				<div class="wrapper">
					<div class="row">
						...
						<div class="col-md-4" id="searchyearTable">
							<table id="example1"
								class="table table-bordered table-striped dataTable dtr-inline">
								<thead>
									<tr>
								...
								<tbody>
									<c:forEach items="${consumeyear}" var="consumeyear">
										<tr class="odd">
											<td class="text-center">${consumeyear.year}년</td>
											<td class="text-center">${consumeyear.totalEnergy}</td>
											<td>${consumeyear.section }</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
							<%@ include file="/WEB-INF/views/energyconsume/yearpagination.jsp" %>
						</div>
					</div>
				</div>
				<br> <br> <br> <br>
...
<script>
   function csvDown() {
      const table = document.getElementById("searchyearTable"); // 테이블 설정
      const rows = table.getElementsByTagName("tr"); // 행 저장
       ...
       link.setAttribute("download", "searchyear.csv"); // alarmLog.csv 다운로드하는 링크
...
function xlsDown(){
    // 대상 테이블을 가져옴
    var table = document.getElementById("searchyearTable");
    
    if(table){
        // CASE 대상 테이블이 존재하는 경우
        
        // 엑셀다운 (엑셀파일명, 시트명, 내부데이터HTML)
        _excelDown("searchyear.xls", "searchyear", table.outerHTML)
    }
}
</script>
