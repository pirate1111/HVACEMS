...
<c:set value="${dataMap.consumeyear}" var="consumeyear" />
...
<body>
						...
						<div class="input-group" style="width: 800px;">
							...
							<input type="text" class="form-control datetimepicker"
								...
								<button type="button" class="btn bg-gradient-success"
									onclick="searchList_go(1);" data-card-widget="analysisyear"
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-12">
						<div class="callout callout-info">
							<h5 style="text-align: center;">${command.sectionType}연도별
								전력소비량 분석</h5>
						</div>
					</div>
				</div>
				...
				<div class="wrapper">
					<div class="row">
									...
									<div class="d-flex justify-content-between mb-4">
										<div class="callout callout-info col-md-6">
											<h4 style="text-align: center; font-size: 18px;">
												가장 많은 전력을 소비한 연도:
												<c:set var="maxConsumeYear" value="" />
												<c:set var="maxConsumeValue" value="0" />
												<c:forEach items="${consumeyear}" var="currentYear"
													varStatus="loop">
													<c:if test="${currentYear.totalEnergy > maxConsumeValue}">
														<c:set var="maxConsumeValue"
															value="${currentYear.totalEnergy}" />
														<c:set var="maxConsumeYear"
															value="${currentYear.year}년" />
													</c:if>
												</c:forEach>
											...
											<h4 style="text-align: center; font-size: 18px;">
												가장 적은 전력을 소비한 연도:
												<c:set var="minConsumeYear" value="" />
												<c:set var="minConsumeValue" value="${maxConsumeValue + 1}" />
												<c:forEach items="${consumeyear}" var="currentYear"
													varStatus="loop">
													<c:if test="${currentYear.totalEnergy < minConsumeValue}">
														<c:set var="minConsumeValue"
															value="${currentYear.totalEnergy}" />
														<c:set var="minConsumeYear"
															value="${currentYear.year}년" />
													</c:if>
												</c:forEach>
												${minConsumeYear}
											...
											<h4 style="text-align: center; font-size: 18px;">
												전력 소비 증가율이 가장 높은 연도:
												<c:set var="maxIncreaseYear" value="" />
												<c:set var="maxIncreaseValue" value="0" />
												<c:forEach items="${consumeyear}" var="currentYear"
													varStatus="loop">
													<c:if test="${loop.index > 0}">
														<c:set var="previousYear"
															value="${consumeYear[loop.index - 1]}" />
														<c:set var="difference"
															value="${currentYear.totalEnergy - previousYear.totalEnergy}" />
														<c:set var="percentageChange"
															value="${previousYear.totalEnergy != 0 ? (difference / previousYear.totalEnergy) * 100 : 0}" />
														<c:if test="${percentageChange > maxIncreaseValue}">
															<c:set var="maxIncreaseValue" value="${percentageChange}" />
															<c:set var="maxIncreaseYear"
																value="${currentYear.year}년" />
														</c:if>
													</c:if>
												</c:forEach>
											...
											<h4 style="text-align: center; font-size: 18px;">
												전력 소비 감소율이 가장 높은 연도:
												<c:set var="maxDecreaseYear" value="" />
										...
										<div class="col-md-6 text-right">
											<table id="example1"
												...
												<thead>
													<tr>
														<th class="sorting text-center" tabindex="0"
															...
															aria-label="Rendering engine: activate to sort column ascending">연
															평균 전력소비량</th>
														<th class="sorting text-center" tabindex="0"
															aria-controls="example1" rowspan="1" colspan="1"
												...
												<tbody>
													...
													<c:if test="${not empty consumeyear}">
														<c:forEach items="${consumeyear}" var="year">
															<c:set var="totalConsumption"
																value="${totalConsumption + year.totalEnergy}" />
														</c:forEach>
														<c:if test="${fn:length(consumeyear) > 0}">
															<c:set var="averageConsumption"
																value="${totalConsumption / fn:length(consumeyear)}" />
														</c:if>
													</c:if>
						...
						<div class="col-md-4" id="analysisYearTable">
							<table id="example1"
								class="table table-bordered table-striped dataTable dtr-inline"
								...
								<tbody>
									<c:forEach items="${consumeyear}" var="currentYear"
										varStatus="loop">
										<tr class="odd">
											<td class="text-center">${currentYear.year}년</td>
											<td class="text-center">${currentYear.totalEnergy}kWh</td>
											<c:if test="${loop.index > 0}">
												<c:set var="previousYear"
													value="${consumeyear[loop.index - 1]}" />
												<c:set var="difference"
							...
							<%@ include
								file="/WEB-INF/views/energyconsume/year_analysis_pagination.jsp"%>
						</div>
					</div>
				</div>
				<br> <br> <br> <br>
    ...
    // JSP 변수 consumeList의 데이터를 그래프 데이터에 추가
     <c:forEach items="${dataMap.consumeyear}" var="consumeyear">
    	 data.labels.push("${consumeyear.year}");
    	 data.datasets[0].data.push(${consumeyear.totalEnergy});
 	</c:forEach>
...
<script>
   function csvDown() {
      const table = document.getElementById("analysisYearTable"); // 테이블 설정
      const rows = table.getElementsByTagName("tr"); // 행 저장
       ...
       link.setAttribute("download", "analysisYear.csv"); // alarmLog.csv 다운로드하는 링크
...
function xlsDown(){
    // 대상 테이블을 가져옴
    var table = document.getElementById("analysisYearTable");
    
    if(table){
        // CASE 대상 테이블이 존재하는 경우
        
        // 엑셀다운 (엑셀파일명, 시트명, 내부데이터HTML)
        _excelDown("analysisYear.xls", "analysisYear", table.outerHTML)
    }
}
</script>
</body>
