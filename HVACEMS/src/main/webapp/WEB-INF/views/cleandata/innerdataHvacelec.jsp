...
<c:set var="temp" value="${dataMap.hvacelecList }"/>
								...
								<div class="card-body">
									<div class="row">
										...
										<div class="col-3" align="right">
											...
											<button type="button" class="btn bg-gradient-success" style="margin-left: 5px; width:74px;" onclick="location.href='hvacelecList.do'">리스트</button>
										</div>
									</div>
									<div class="row">
										<div class="col-12" id="chartCanvas">
											<canvas id="hvacelecDetailChart" style="width:100%; height:700px;"></canvas>
										</div>
									</div>
								</div>
							</div>
...
// 			alert(data[0].JSONTemperature);
			for(let i = 0; i < data.length; i++){
				timeList.push(data[i].date);
				hvacelecList.push(data[i].hvacelecData);
				sdhvacelecList.push(1000)
			}

			new Chart(document.getElementById("hvacelecDetailChart"), {
			    type: 'bar',
			    data: {
			    	labels: timeList,	
			    	datasets: [{
