...
<c:set var="temp" value="${dataMap.temperatureList }"/>
					...
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-body">
									<div class="row">
										...
										<div class="col-3" align="right">
											...
											<button type="button" class="btn bg-gradient-success" style="margin-left: 5px; width:74px;" onclick="location.href='temp.do'">그래프</button>
										</div>
									</div>
									<br/>
									
									<div class="row">
										<div class="col-12" id="tempTable">
											<table class="table table-bordered" >
												<thead>
													<tr>
														<th class="text-center" style="width:25%;">온도번호</th>
														<th class="text-center" style="width:25%;">온도날짜</th>
														<th class="text-center" style="width:25%;">온도(℃)</th>
														<th class="text-center" style="width:25%;">구역</th>
													</tr>
												</thead>
												<c:if test="${empty dataMap.temperatureList }">
													<tr>
														<td colspan="4" class="text-center"><strong>해당 내용이 없습니다.</strong></td>
													</tr>
												</c:if>
												<tbody>
												<c:forEach items="${dataMap.temperatureList}" var="temp">
													<tr>
														<td class="text-center">${temp.temperatureNum }</td>
														<td class="text-center"><fmt:formatDate value="${temp.temperatureDate }" pattern="yyyy-MM-dd HH:mm" /></td>
														<td class="text-center">${temp.temperature }</td>
														<td class="text-center">
															<c:choose>
																<c:when test="${temp.thermometerNum eq 'A00'}">${temp.thermometerNum.replace('A00','전체구역') }</c:when>
																<c:otherwise>${temp.thermometerNum.replace('01','구역') }</c:otherwise>
															</c:choose>
														</td>
													</tr>
												</c:forEach>
								...
								<%@ include file="/WEB-INF/views/cleandata/temppagination.jsp" %>
								</div>
							</div>
						</div>
					</div>
...
<script>
   function downloadCSV() {
      const table = document.getElementById("tempTable"); // 테이블 설정
      const rows = table.getElementsByTagName("tr"); // 행 저장
       ...
       link.setAttribute("download", "tempList.csv"); // alarmLog.csv 다운로드하는 링크
...
function downloadPDF() { 
    ...
    html2canvas($('#tempTable')[0]).then(function(canvas) { 
...
function downloadXLS(){
    ...
    if(table){
        ...
        _excelDown("tempList.xls", "tempList", table.outerHTML)
    }
}
</script>
</body>