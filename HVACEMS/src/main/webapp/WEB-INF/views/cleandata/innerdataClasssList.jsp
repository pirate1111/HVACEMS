					...
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-body">
									<div class="row">
										...
										<div class="col-3" align="right">
											...
											<button type="button" class="btn bg-gradient-success" style="margin-left: 5px; width:74px;" onclick="location.href='classs.do'">그래프</button>
										</div>
									</div>
									<br/>
									
									<div class="row">
										<div class="col-12" id="classsTable" >
											<table class="table table-bordered" >
												...
												<c:if test="${empty dataMap.classsList }">
													<tr>
														<td colspan="4" class="text-center"><strong>해당 내용이 없습니다.</strong></td>
													</tr>
												</c:if>
												<tbody>
												<c:forEach items="${dataMap.classsList}" var="classs">
													<tr>
														<td class="text-center">${classs.classsNum }</td>
														<td class="text-center"><fmt:formatDate value="${classs.classsDate }" pattern="yyyy-MM-dd HH:mm" /></td>
														<td class="text-center">${classs.classs }</td>
														<td class="text-center">
															<c:choose>
																<c:when test="${classs.dustmeterNum eq 'A00'}">${classs.dustmeterNum.replace('A00','전체구역') }</c:when>
																<c:otherwise>${classs.dustmeterNum.replace('01','구역') }</c:otherwise>
															</c:choose>
														</td>
													</tr>
												</c:forEach>
								...
								<%@ include file="/WEB-INF/views/cleandata/classspagination.jsp" %>
								</div>
							</div>
						</div>
					</div>
...
<script>
   function downloadCSV() {
      const table = document.getElementById("classsTable"); // 테이블 설정
      const rows = table.getElementsByTagName("tr"); // 행 저장
       ...
       link.setAttribute("download", "classList.csv"); // alarmLog.csv 다운로드하는 링크
...
function downloadPDF() { 
    ...
    html2canvas($('#classsTable')[0]).then(function(canvas) { 
...
function download(){
    // 대상 테이블을 가져옴
    var table = document.getElementById("classsTable");
    
    if(table){
        // CASE 대상 테이블이 존재하는 경우
        
        // 엑셀다운 (엑셀파일명, 시트명, 내부데이터HTML)
        _excelDown("classList.xls", "classList", table.outerHTML)
    }
}
</script>
</body>