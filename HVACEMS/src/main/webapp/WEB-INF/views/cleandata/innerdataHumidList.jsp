...
<c:set var="humid" value="${dataMap.humidList }"/>
					...
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-body">
									<div class="row">
										...
										<div class="col-3" align="right">
											...
											<button type="button" class="btn bg-gradient-success" style="margin-left: 5px; width:74px;" onclick="location.href='humid.do'">그래프</button>
										</div>
									</div>
									<br/>
									
									<div class="row">
										<div class="col-12" id="humidTable" >
											<table class="table table-bordered" >
												<thead>
													<tr>
														<th class="text-center" style="width:25%;">습도번호</th>
														<th class="text-center" style="width:25%;">습도날짜</th>
														<th class="text-center" style="width:25%;">습도(%)</th>
														<th class="text-center" style="width:25%;">구역</th>
													</tr>
												</thead>
												<c:if test="${empty dataMap.humidList }">
													<tr>
														<td colspan="4" class="text-center"><strong>해당 내용이 없습니다.</strong></td>
													</tr>
												</c:if>
												<tbody>
												<c:forEach items="${dataMap.humidList}" var="humid">
													<tr>
														<td class="text-center">${humid.humidNum }</td>
														<td class="text-center"><fmt:formatDate value="${humid.humidDate }" pattern="yyyy-MM-dd HH:mm" /></td>
														<td class="text-center">${humid.humid }</td>
														<td class="text-center">
															<c:choose>
																<c:when test="${humid.hygrometerNum eq 'A00'}">${humid.hygrometerNum.replace('A00','전체구역') }</c:when>
																<c:otherwise>${humid.hygrometerNum.replace('01','구역') }</c:otherwise>
															</c:choose>
														</td>
													</tr>
												</c:forEach>
								...
								<%@ include file="/WEB-INF/views/cleandata/humidpagination.jsp" %>
								</div>
							</div>
						</div>
					</div>
...
<script>
   function downloadCSV() {
      const table = document.getElementById("humidTable"); // 테이블 설정
      const rows = table.getElementsByTagName("tr"); // 행 저장
       ...
       link.setAttribute("download", "humidList.csv"); // alarmLog.csv 다운로드하는 링크
...
function downloadPDF() { 
    ...
    html2canvas($('#humidTable')[0]).then(function(canvas) { 
...
function downloadXLS(){
    // 대상 테이블을 가져옴
    var table = document.getElementById("humidTable");
    
    if(table){
        // CASE 대상 테이블이 존재하는 경우
        
        // 엑셀다운 (엑셀파일명, 시트명, 내부데이터HTML)
        _excelDown("humidList.xls", "humidList", table.outerHTML)
    }
}
</script>
</body>