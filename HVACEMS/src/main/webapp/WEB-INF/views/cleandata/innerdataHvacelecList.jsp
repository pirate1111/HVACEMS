...
<c:set var="hvacelec" value="${dataMap.hvacelecList }"/>
					...
					<div class="row">
						<div class="col-12">
							<div class="card">
									...
									<div class="row">
										<div class="col-3">
											<div class="dt-buttons btn-group flex-wrap ">
												...
												<button type="button" class="btn bg-gradient-success" style="width:74px;" onclick="download();">EXCEL</button>
											</div>
										</div>
										<div class="col-6">
											<div class="row">
										...
										<div class="col-3" align="right">
											...
											<button type="button" class="btn bg-gradient-success" style="margin-left: 5px; width:74px;" onclick="location.href='hvacelec.do'">그래프</button>
										</div>
									</div>
									<br/>
									
									<div class="row">
										<div class="col-12" id="hvacelecTable">
											<table class="table table-bordered"  >
												<thead>
													<tr>
														<th class="text-center" style="width:25%;">전력량번호</th>
														<th class="text-center" style="width:25%;">전력량날짜</th>
														<th class="text-center" style="width:25%;">전력량(kWh)</th>
														<th class="text-center" style="width:25%;">구역</th>
													</tr>
												</thead>
												<c:if test="${empty dataMap.hvacelecList }">
													<tr>
														<td colspan="4" class="text-center"><strong>해당 내용이 없습니다.</strong></td>
													</tr>
												</c:if>
												<tbody>
												<c:forEach items="${dataMap.hvacelecList}" var="hvacelec">
													<tr>
														<td class="text-center">${hvacelec.hvacelecdataNum }</td>
														<td class="text-center"><fmt:formatDate value="${hvacelec.hvacelecDate }" pattern="yyyy-MM-dd HH:mm" /></td>
														<td class="text-center">${hvacelec.hvacelecData }</td>
														<td class="text-center">
															<c:choose>
																<c:when test="${hvacelec.hvacelecSection eq 'A00'}">${hvacelec.hvacelecSection.replace('A00','전체구역') }</c:when>
																<c:otherwise>${hvacelec.hvacelecSection.replace('01','구역') }</c:otherwise>
															</c:choose>
														</td>
													</tr>
												</c:forEach>
								...
								<%@ include file="/WEB-INF/views/cleandata/hvacelecpagination.jsp" %>
								</div>
							</div>
						</div>
					</div>
...
<script>
   function downloadCSV() {
      const table = document.getElementById("hvacelecTable"); // 테이블 설정
      const rows = table.getElementsByTagName("tr"); // 행 저장
       ...
       link.setAttribute("download", "hvacelecList.csv"); // alarmLog.csv 다운로드하는 링크
...
function downloadPDF() { 
    ...
    html2canvas($('#hvacelecTable')[0]).then(function(canvas) { 
...
function downloadXLS(){
    // 대상 테이블을 가져옴
    var table = document.getElementById("hvacelecTable");
    
    if(table){
        // CASE 대상 테이블이 존재하는 경우
        
        // 엑셀다운 (엑셀파일명, 시트명, 내부데이터HTML)
        _excelDown("hvacelecList.xls", "hvacelecList", table.outerHTML)
    }
}
</script>
</body>